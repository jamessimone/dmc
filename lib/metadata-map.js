...
Map.prototype.addFiles = function (files) {
  ...
  _.each(files, function (f) {
    ...
    if (existing) {
      ...
      if (self.meta[parsed.type.name]) {
        self.meta[parsed.type.name].push(md);
      } else self.meta[parsed.type.name] = [md];
    }

    //md.fileName = parsed.fileName;
    md.name = parsed.name;
    md.path = parsed.srcPath;
...
Map.prototype.fetchIds = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    sfClient
      .getClient(self.oauth)
      .then(function (sfdcclient) {
        client = sfdcclient;
        async.concat(self.getTypesForQuery(), iterator, function (
          err,
          results
        ) {
...
Map.prototype.createPackageXML = function (version) {
  ...
  function getTypes() {
    ...
    _(self.meta)
      .map(function (metaList, metaType) {
...
Map.prototype.requiresMetadataDeploy = function () {
  ...
  _.forOwn(self.meta, function (value, key) {
    if (_.isArray(value) && value.length > 0) {
      ...
      if (
        _.indexOf(type.deployTypes, "metadata") !== -1 &&
