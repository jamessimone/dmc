      ...
      var tests = _(res.records)
        .map(function (r) {
          ...
          if (r.SymbolTable == null) {
            if (!r.IsValid) {
              logger.error(
                "Dependent class is invalid and needs recompilation: " + r.Name
              );
            }
          } else {
...
function runTests(client, tests, opts) {
  return new Promise(function (resolve, reject) {
    ...
    client.tooling.runTestsAsyncPost({ ids: testIds.join(",") }, function (
      err,
      res
    ) {
      if (err) return reject(err);
...
var run = (module.exports.run = function (opts, cb) {
  ...
  Promise.resolve()
    ...
    .then(function (matchedTests) {
      ...
      return runTests(client, matchedTests, opts).then(function (tests) {
        var failedTests = 0;
        ...
        _.each(tests.records, function (t) {
          if (!currentClass || currentClass != t.ApexClass.Name) {
            ...
            logger.log(
              openingDelim + t.ApexClass.Name + " test results" + closingDelim
            );
          }
        ...
        logger.log(
          openingDelim + "Number of tests run: " + numberOfTests + closingDelim
        );
        logger.log(
          openingDelim +
            "Total test time: " +
